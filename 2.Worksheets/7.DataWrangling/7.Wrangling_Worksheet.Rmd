...
author: "Sizek Z620: Quantitative Biodiversity, Indiana University"
...
rm(list=ls()) 
getwd() 
setwd("D:/GitHub/QB2021_Sizek/2.Worksheets/7.DataWrangling")
```
...
comA <- sim_poisson_community(s_pool = 100, n_sim = 10000, sad_type = "lnorm", 
        sad_coef = list("meanlog" = 2, "sdlog" = 1))
# visualize site
#plot(com1)
#Generate Quadrats
q_areaMean <-function(community,q_area,q_n){
  comm_matA <- sample_quadrats(community, n_quadrats = q_n, quadrat_area = q_area, 
             method = "grid", avoid_overlap = F, plot=F);
  row.names(comm_matA$xy_dat) <- quads;
  row.names(comm_matA$spec_dat) <- quads;
  return( mean(specnumber(comm_matA$spec_dat)))
}
q_areas <- 10^seq(-4.5,-1,0.01)
# q_areas <- seq(0.0001,0.01,0.0001)
qAreaMeans <-vector("list", length(q_areas))
for (i in 1:length(q_areas)){
  qAreaMeans[i] <- q_areaMean(comA,q_areas[i],10)
}
...
```{r}
qlm <- lm(log(unlist(qAreaMeans),base=10)~log(q_areas,base=10))
plot(log(q_areas,base=10),log(unlist(qAreaMeans),base=10))
lines(log(q_areas,base=10),log(q_areas,base=10)*qlm$coefficients[2]+qlm$coefficients[1])
``` 

> The estimated z value is:

```{r}
qlm$coefficients[2]
``` 

> This is an underestimation of the true z-value, because we are reaching the saturation of quadrats (i.e. we are detecting all of the species present in the quadrats.) Depending on the purposes it might be better to estimate z prior to the saturation threshold. 
...
```{r warning=FALSE}
comB <- sim_thomas_community(s_pool = 100, n_sim = 10000, sad_type = "lnorm", 
        sad_coef = list("meanlog" = 2, "sdlog" = 1))
for (i in 1:length(q_areas)){
  qAreaMeans[i] <- q_areaMean(comB,q_areas[i],10)
}
``` 
...
qlm$coefficients[2]
``` 
> This is lower than the z value for the random distribution. Note that this is less likely to be an underestimate because the species have not been saturated yet. 
...
> First, we can see that in the random distribution, the sampling is saturating, while in the thompson model, the sampling is not saturating yet. This makes sense, as the clusting of organisms limits the organisms range more, so it is less likely to stumble upon them randomly. Depending on the purpose it might be better to have quadrats on a latice instead of random placement. I am somewhat interested in using the Forest GEO datasets to develop methods on detecting interspecies clustering. In that case, for the simulated model, we would have to have an interspecies associvity (or dissociativity) term. The thompson model would provide a random null type model, that could be used as a baseline of what the impact of random clusting is on apparent species associations.  
